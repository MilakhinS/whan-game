...
export default function HomePage() {
  ...
  const gbtn = (active=true, extra={}) => ({ padding:'10px 22px', borderRadius:12, border:`1px solid ${active?T.gold:T.goldDim+'44'}`, background:active?`linear-gradient(135deg,${T.goldDim}88,${T.goldDim}cc,${T.goldDim}88)`:'rgba(255,255,255,0.03)', color:active?T.text:T.goldDim, cursor:active?'pointer':'not-allowed', fontSize:16, fontWeight:600, boxShadow:active?`0 2px 16px ${T.goldGlow}`:'none', transition:'all 0.18s', ...extra } as any)
  const inp = { background:'rgba(255,255,255,0.04)', border:`1px solid ${T.goldDim}55`, borderRadius:12, padding:'10px 14px', color:T.text, fontSize:16, outline:'none', width:'100%', boxSizing:'border-box' as const, fontFamily:'inherit' }
  const badge = (color=T.gold) => ({ display:'inline-block', padding:'2px 10px', borderRadius:20, border:`1px solid ${color}44`, background:`${color}18`, color, fontSize:17, fontWeight:600 })
        ...
        {pendingRoom && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}
            ...
            <motion.div initial={{scale:0.85,y:20}} animate={{scale:1,y:0}}
              ...
              <div style={{ fontSize:17, color:`${T.gold}88`, letterSpacing:2, marginBottom:6 }}>ВХОД В КОМНАТУ</div>
              ...
              {passError && <div style={{ fontSize:16, color:'#e74c3c', marginBottom:10 }}>{passError}</div>}
        ...
        {activeTab==='rooms' && (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            ...
            {activeGames.length > 0 && (
              <div>
                <div style={{ fontSize:16, color:`${T.gold}66`, letterSpacing:2, marginBottom:8, display:'flex', alignItems:'center', gap:8 }}>
                    ...
                    <motion.div key={room.id} whileHover={{scale:1.01}}
                      ...
                      <div style={{ flex:1, minWidth:0 }}>
                        <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:4 }}>
                          <div style={{ fontSize:17, fontWeight:600, color:T.text, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{room.name}</div>
                          <span style={{ fontSize:17, color:'#27ae60', background:'rgba(39,174,96,0.15)', border:'1px solid rgba(39,174,96,0.3)', borderRadius:10, padding:'1px 6px' }}>В игре</span>
                        </div>
                      ...
                      <button onClick={()=>joinActiveGame(room)}
                        style={{ padding:'8px 14px', borderRadius:10, cursor:'pointer', fontFamily:'inherit', fontSize:16, fontWeight:600, border:'1px solid rgba(39,174,96,0.4)', background:'rgba(39,174,96,0.12)', color:'#27ae60', flexShrink:0 }}>
                        Войти
                      </button>
                    </motion.div>
                  ))}
            ...
            {/* Waiting rooms */}
            {filtered.length===0 ? (
              <div style={{ textAlign:'center', color:`${T.gold}33`, fontSize:17, padding:'40px 0', fontStyle:'italic' }}>
              ...
              <motion.div key={room.id} whileHover={{scale:1.01}}
                ...
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:6 }}>
                    <div style={{ fontSize:16, fontWeight:600, color:T.text, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{room.name}</div>
                ...
                <button onClick={()=>joinRoom(room)} disabled={room.player_count>=room.max_players} style={gbtn(room.player_count<room.max_players,{padding:'10px 16px',fontSize:17,flexShrink:0})}>
        ...
        {activeTab==='create' && (
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            <div style={{ ...panel(), padding:'20px 16px' }}>
              <div style={{ fontSize:17, color:`${T.gold}88`, letterSpacing:2, marginBottom:14 }}>НОВАЯ КОМНАТА</div>
              <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                  {([['team','2 × 2','4 игрока'],['solo','Сам за себя','до 6']] as const).map(([mode,title,sub])=>(
                    <button key={mode} onClick={()=>setGameMode(mode)} style={{ padding:'14px 6px', borderRadius:12, cursor:'pointer', fontFamily:'inherit', textAlign:'center', border:`1.5px solid ${gameMode===mode?T.gold:T.goldDim+'33'}`, background:gameMode===mode?`${T.gold}18`:'rgba(255,255,255,0.02)', color:gameMode===mode?T.gold:T.goldDim, transition:'all 0.18s' }}>
                      ...
                      <div style={{fontSize:16,opacity:0.6,marginTop:2}}>{sub}</div>
                    </button>
                  ))}
                </div>
                <button onClick={createRoom} disabled={loading} style={gbtn(!loading,{padding:'14px',fontSize:17})}>
                  {loading?'Создаём…':'🎴 Создать комнату'}
                </button>
              </div>
            </div>
            <div style={{ ...panel(), padding:'16px' }}>
              <div style={{ fontSize:17, color:`${T.gold}66`, letterSpacing:2, marginBottom:10 }}>ПРАВИЛА</div>
              <div style={{ fontSize:16, color:`${T.gold}88`, lineHeight:2 }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
            {profile && myRank >= 0 && (
              <div style={{ ...panel({border:`1px solid ${T.gold}44`}), padding:'14px 16px', display:'flex', alignItems:'center', gap:12 }}>
                ...
                <div style={{ flex:1 }}>
                  <div style={{ fontSize:16, fontWeight:700, color:T.gold }}>{profile.username} <span style={{ fontSize:17, color:`${T.gold}66` }}>(вы)</span></div>
                  <div style={{ fontSize:17, color:`${T.gold}66`, marginTop:2 }}>{profile.wins} побед · {profile.streak} серия</div>
                </div>
                <div style={{ textAlign:'right' }}>
                  <div style={{ fontSize:22, fontWeight:700, color:T.gold }}>{profile.mmr}</div>
                  <div style={{ fontSize:16, color:`${T.gold}44` }}>MMR</div>
                </div>
              </div>
            )}
            ...
            {leaders.length >= 3 && (
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1.2fr 1fr', gap:8, marginBottom:4 }}>
                <div style={{ ...panel({border:`1px solid ${T.goldDim}33`}), padding:'14px 8px', textAlign:'center', alignSelf:'flex-end' }}>
                  ...
                  <div style={{ fontSize:16, fontWeight:600, color:T.text, marginTop:6, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{leaders[1]?.username}</div>
                ...
                <div style={{ ...panel({border:`1px solid ${T.gold}55`, background:`${T.gold}11`}), padding:'20px 8px', textAlign:'center' }}>
                  ...
                  <div style={{ fontSize:17, fontWeight:700, color:T.gold, marginTop:6, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{leaders[0]?.username}</div>
                  <div style={{ fontSize:22, fontWeight:700, color:T.gold, marginTop:4 }}>{leaders[0]?.mmr}</div>
                </div>
                <div style={{ ...panel({border:`1px solid ${T.goldDim}33`}), padding:'14px 8px', textAlign:'center', alignSelf:'flex-end' }}>
                  <div style={{ fontSize:22 }}>🥉</div>
                  <div style={{ fontSize:16, fontWeight:600, color:T.text, marginTop:6, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{leaders[2]?.username}</div>
              ...
              {leaders.map((player,i)=>{
                ...
                return (
                  <motion.div key={player.id} initial={{opacity:0,x:-10}} animate={{opacity:1,x:0}} transition={{delay:i*0.03}}
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontSize:17, fontWeight:isMe?700:500, color:isMe?T.gold:T.text, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                        {player.username}{isMe?' 👈':''}
                      </div>
                      <div style={{ fontSize:16, color:`${T.gold}55`, marginTop:1 }}>{player.wins}W · {player.losses}L · {player.streak}🔥</div>
                    </div>
                    <div style={{ textAlign:'right', flexShrink:0 }}>
                      <div style={{ fontSize:16, fontWeight:700, color:i===0?T.gold:i===1?'#c0c0c0':i===2?'#cd7f32':T.text }}>{player.mmr}</div>
                      <div style={{ fontSize:17, color:`${T.gold}44` }}>MMR</div>
                    </div>
                  </motion.div>
                )
              })}
              {leaders.length===0 && <div style={{ textAlign:'center', color:`${T.gold}33`, fontSize:17, padding:'60px 0', fontStyle:'italic' }}>Сыграй первым! 🎴</div>}
            </div>
          </div>
        )}
        ...
        {activeTab==='profile' && profile && (
          <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <div style={{ ...panel(), padding:'20px 16px', textAlign:'center' }}>
              <div style={{ width:72, height:72, borderRadius:'50%', background:`linear-gradient(135deg,${T.goldDim},${T.gold})`, margin:'0 auto 12px', display:'flex', alignItems:'center', justifyContent:'center', fontSize:26, fontWeight:700, color:'#000', boxShadow:`0 0 24px ${T.goldGlow}` }}>
                {profile.username[0]?.toUpperCase()}
              </div>
              <div style={{ fontSize:16, fontWeight:700, color:T.gold }}>{profile.username}</div>
              <div style={{ fontSize:17, color:`${T.gold}55`, marginTop:4 }}>MMR {profile.mmr}</div>
            </div>

            {/* Edit username */}
            <div style={{ ...panel(), padding:'18px 16px' }}>
              <div style={{ fontSize:17, color:`${T.gold}88`, letterSpacing:2, marginBottom:12 }}>ИЗМЕНИТЬ НИКНЕЙМ</div>
              <input style={inp} placeholder="Новый никнейм" value={editUsername} onChange={e=>setEditUsername(e.target.value)} maxLength={20}/>
              <div style={{ fontSize:16, color:`${T.gold}44`, marginTop:6 }}>{editUsername.length}/20 символов</div>
            </div>

            {/* Theme selector */}
            <div style={{ ...panel(), padding:'18px 16px' }}>
              <div style={{ fontSize:17, color:`${T.gold}88`, letterSpacing:2, marginBottom:14 }}>ТЕМА ОФОРМЛЕНИЯ</div>
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {(Object.entries(THEMES) as [ThemeName, typeof THEMES[ThemeName]][]).map(([key, theme])=>(
                  <button key={key} onClick={()=>applyTheme(key)}
                    ...
                    {/* Color preview */}
                    <div style={{ width:36, height:36, borderRadius:10, background:theme.bg, border:`1px solid ${theme.gold}66`, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center', fontSize:16 }}>
                      {theme.emoji}
                    </div>
                    <div>
                      <div style={{ fontSize:16, fontWeight:600, color:theme.gold }}>{theme.name}</div>
                      <div style={{ fontSize:17, color:`${theme.gold}66`, marginTop:2 }}>
            ...
            {/* Save button */}
            <button onClick={saveProfile} disabled={savingProfile} style={gbtn(!savingProfile,{padding:'14px',fontSize:17,width:'100%'})}>
              {savingProfile?'Сохраняем…':'💾 Сохранить'}
            </button>

            {/* Stats */}
            <div style={{ ...panel(), padding:'16px' }}>
              <div style={{ fontSize:17, color:`${T.gold}88`, letterSpacing:2, marginBottom:12 }}>СТАТИСТИКА</div>
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8 }}>
                {[['🏆 Победы',profile.wins],['💀 Поражения',profile.losses],['🔥 Серия',profile.streak],['⭐ MMR',profile.mmr]].map(([lbl,val])=>(
                  <div key={String(lbl)} style={{ background:'rgba(255,255,255,0.02)', border:`1px solid ${T.goldDim}33`, borderRadius:12, padding:'12px', textAlign:'center' }}>
                    <div style={{ fontSize:16, color:`${T.gold}66` }}>{lbl}</div>
      ...
      <div style={{ position:'fixed', bottom:0, left:'50%', transform:'translateX(-50%)', width:'100%', maxWidth:600, background:`${T.bgSolid}f5`, borderTop:`1px solid ${T.goldDim}33`, backdropFilter:'blur(12px)', zIndex:50, display:'grid', gridTemplateColumns:'1fr 1fr 1fr 1fr', padding:'8px 12px 12px', gap:6 }}>
        {([['rooms','🎮','Комнаты'],['create','➕','Создать'],['leaders','🏆','Рейтинг'],['profile','👤','Профиль']] as const).map(([tab,icon,lbl])=>(
          <button key={tab} onClick={()=>{ setActiveTab(tab); if(tab==='leaders') loadLeaders() }} style={{ padding:'8px', borderRadius:12, cursor:'pointer', fontFamily:'inherit', border:'none', background:activeTab===tab?`${T.gold}22`:'transparent', color:activeTab===tab?T.gold:`${T.gold}44`, transition:'all 0.18s', display:'flex', flexDirection:'column', alignItems:'center', gap:2 }}>
            ...
            <span style={{ fontSize:17, fontWeight:activeTab===tab?700:400 }}>{lbl}</span>
          </button>
        ))}
      </div>

      {toast && (
        <div style={{ position:'fixed', top:14, left:'50%', transform:'translateX(-50%)', background:`linear-gradient(135deg,${T.goldDim}cc,${T.goldDim})`, color:T.text, borderRadius:10, padding:'10px 22px', fontSize:17, fontWeight:600, zIndex:999, border:`1px solid ${T.gold}`, boxShadow:`0 0 24px ${T.goldGlow}`, whiteSpace:'nowrap' }}>
          {toast}
        </div>
      )}
